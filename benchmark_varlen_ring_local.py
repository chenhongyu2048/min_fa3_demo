...
Examples:
    ...
    python benchmark_varlen_ring_local.py --b 4 --seqlen 512,1024,2048 --qhead 32 --kvhead 8 --headdim 128 --mode both
    python benchmark_varlen_ring_local.py --b 4 --seqlen 1024 --qhead 32 --kvhead 8 --num-comp-sm 128 --num-comm-sm 4 --mode causal
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark min_fa3 varlen ring-local demo against PyTorch / FA2 / FA3 / base min_fa3 varlen"
    )
    ...
    parser.add_argument("--num-comp-sm", type=int, default=128, help="Number of compute CTAs for min_fa3_varlen_ring")
    parser.add_argument(
        "--num-comm-sm",
        type=int,
        default=0,
...
def make_ring_aux(
    k: torch.Tensor,
    v: torch.Tensor,
) -> tuple[min_fa3_op.TKParallelTensor, min_fa3_op.TKParallelTensor, torch.Tensor, torch.Tensor]:
    local_rank = torch.cuda.current_device()
    remote_k = min_fa3_op.create_parallel_tensor(k, local_rank=local_rank, local_world_size=1)
    remote_v = min_fa3_op.create_parallel_tensor(v, local_rank=local_rank, local_world_size=1)
    prefetch_k = torch.empty_like(k)
    prefetch_v = torch.empty_like(v)
    return remote_k, remote_v, prefetch_k, prefetch_v
...
def bench_case(
    ...
    try:
        ...
        remote_k, remote_v, prefetch_k, prefetch_v = make_ring_aux(k, v)
...
def print_table(results_by_case: dict[Case, dict[str, Result | None]]) -> None:
    methods = ["PyTorch_SDPA", "FA2_varlen", "FA3_varlen", "min_fa3_varlen", "min_fa3_varlen_ring"]
    print("\n" + "=" * 100)
    print("Varlen Ring-Local Benchmark Results")
    print("=" * 100)
