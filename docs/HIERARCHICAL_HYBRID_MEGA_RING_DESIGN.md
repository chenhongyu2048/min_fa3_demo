...
与当前实现约束；完整 G8/G4/G2/G1 层级需要 `world_size=8`。
...
不再提供单行或非对齐 tail fallback。当前约束以
[根 README](../README.md) 为准。
...
当前固定 16-row 2D TMA subtile 的每一行包含 1024 个 BF16，因此额外要求：
...
ring_starts_host[b] + ring_sizes_host[b] <= world_size
...
```text
rank_kv_capacity = max(local_total_k across all physical ranks)
```
...
- `world_size` 不属于 2/4/8
- ring size 不属于 1/2/4/8
- ring start 未按 ring size 对齐
- ring 越过 physical world size
