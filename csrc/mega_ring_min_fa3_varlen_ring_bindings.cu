...
py::object forward_varlen_mega_ring(torch::Tensor q,
    ...
    TORCH_CHECK(num_comm_sm > 0 || reduction_tiles == 0,
                "num_comm_sm must be positive when this rank has hierarchical CP replay work");
...
py::tuple forward_varlen_mega_ring_ablation(
    ...
    TORCH_CHECK((remote_k.local_world_size_ == 2
                    || remote_k.local_world_size_ == 4
                    || remote_k.local_world_size_ == 8)
                    && remote_v.local_world_size_ == remote_k.local_world_size_,
                "forward ablation requires CP world size 2, 4, or 8");
...
void bind_varlen_mega_ring(py::module_& m) {
    m.def(
        ...
        "Preallocated causal CP2/CP4/CP8 forward-ablation runner.");
    m.def(
