...
void run_min_fa3_kvcache_combine(
    ...
    if (params.num_splits <= 32) {
        run_min_fa3_kvcache_combine_sm90<5>(params, stream, enable_pdl);
    } else if (params.num_splits <= 64) {
        run_min_fa3_kvcache_combine_sm90<6>(params, stream, enable_pdl);
    } else {
        run_min_fa3_kvcache_combine_sm90<7>(params, stream, enable_pdl);
    }
}
