...
import min_fa3_op
from allgather_attention import AllGatherAttention, Llama3AllGatherAttention, repartition_sequence_shards_to_llama3, select_fa3_backend
...
from ring_common import RingComm, ring_varlen_forward, update_out_and_lse, zigzag_ring_varlen_forward
...
def _serialized_ring(q, k, v, cu, cu_host, max_len, causal):
    comm = RingComm(dist.group.WORLD)
    current_k, current_v = k, v
    out = None
    lse = None
    for step in range(comm.world_size):
        block_out, block_lse = _min_fa3_block_varlen(
            q, current_k, current_v, cu, cu, cu_host, cu_host,
            max_len, max_len, causal and step == 0,
        )
...
def main() -> None:
    ...
    global_seqlens = [args.seqlen * world_size] * args.b
    llama_q = repartition_sequence_shards_to_llama3(dist.group.WORLD, q, global_seqlens, causal)
    llama_k = repartition_sequence_shards_to_llama3(dist.group.WORLD, k, global_seqlens, causal)
    llama_v = repartition_sequence_shards_to_llama3(dist.group.WORLD, v, global_seqlens, causal)
    llama3 = Llama3AllGatherAttention(
        dist.group.WORLD, llama_q, llama_k, llama_v, global_seqlens, causal, backend,
        heads_k_stride=1,
    )
    ...
    def ring_overlap():
        def block(q_, k_, v_, causal_):
            return _min_fa3_block_varlen(
                q_, k_, v_, cu, cu, cu_host, cu_host,
                args.seqlen, args.seqlen, causal_,
            )
        if causal:
            return zigzag_ring_varlen_forward(
                dist.group.WORLD, q, k, v, cu, cu_host, args.seqlen,
                _min_fa3_block_varlen,
            )
        return ring_varlen_forward(dist.group.WORLD, q, k, v, False, block)
    ...
    def serial_allgather():
        return allgather.forward()

    def local_compute():
        local_k = remote_k.data_[rank * capacity : rank * capacity + total]
        local_v = remote_v.data_[rank * capacity : rank * capacity + total]
        result = None
        visible_steps = world_size if not causal else rank + 1
        for step in range(visible_steps):
            result = min_fa3_op.forward_varlen(
                q, local_k, local_v, cu, cu,
                args.seqlen, args.seqlen, causal and step == 0,
                cu_seqlens_q_host=cu_host, cu_seqlens_k_host=cu_host,
                return_lse=True,
            )
        return result
    ...
    if "ring" in requested:
        calls.update({"ring_comm_only": lambda: _communication_only(k, v), "ring_comp_only": local_compute, "ring_serial": lambda: _serialized_ring(q, k, v, cu, cu_host, args.seqlen, causal), "ring_overlap": ring_overlap})
    if "allgather" in requested:
        calls.update({"allgather_comm_only": allgather_comm, "allgather_comp_only": local_compute, "allgather_serial": lambda: allgather.forward(overlap=False), "allgather_overlap": allgather_call})
    if "llama3" in requested:
        calls["llama3_overlap"] = llama3.forward
        calls["llama3_serial"] = lambda: llama3.forward(overlap=False)
