...
keeps the scope to step_external_reduce, step_fused_reduce and
linear_queue_recycle, with K/V preloaded before compute-only timing.
"""
...
from motivation.forward_ablation import ForwardAblationPlan, make_cu_seqlens as make_ablation_cu
...
def main() -> None:
    ...
    if world_size not in (4, 8):
        raise SystemExit("motivation T2 requires CP world size 4 or 8")
    ...
    for profile_name in (
        ...
        plan = ForwardAblationPlan(
            ...
            collect_stats=False, compute_only=True,
        )
        timing = timed_call(plan.run, args.warmup, args.iters, device)
        output = plan.run()[0].clone()
        if results:
        ...
        plan = ForwardAblationPlan(
            q, remote_k, remote_v, cu, cu_host, args.seqlen,
            [args.seqlen * world_size] * args.b, [world_size] * args.b, [0] * args.b,
            profile_name, num_comp_sm=args.num_comp_sm, num_comm_sm=0,
    ...
    payload = {
        "schema": "motivation.v2.T2", **provenance(),
        "schema_version": 2,
