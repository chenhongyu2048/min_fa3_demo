...
INPUT_PATH = THIS_DIR / "min_fa3_demo_h200-107669.out"
RUN_ID = INPUT_PATH.stem.rsplit("-", 1)[-1]

WORLD_SIZE = 4
CHECK = False
SEQLENS = [512, 1024, 2048, 4096, 8192, 16384, 32768, 65536]
PANEL_CONFIGS = [
    (128, 4),
    (124, 8),
    (116, 16),
...
# MODES = [True, False]
MODES = [True,]  # Only plot causal mode for now
...
def _format_seqlen_label(seqlen: int) -> str:
...
RESULT_RE = re.compile(
    r"^(\S+)\s+"
    r"(?:t\d+=\d+(?:\.\d+)?,\s+)*"
    r"t\d+=\d+(?:\.\d+)?\s+\|\s+max_across_ranks=(\d+(?:\.\d+)?)\s+"
    r"(\d+(?:\.\d+)?)\s+"
    r"(\d+(?:\.\d+)?)\s+"
    r"\S+"
)
...
def _make_figure(metric_key: str, title: str):
    ...
    for col_idx, config in enumerate(PANEL_CONFIGS):
        for row_idx, is_causal in enumerate(MODES):
            ...
            ax.set_xlim(SEQLENS[0], SEQLENS[-1])
            ax.set_xticks(SEQLENS)
            ax.set_xticklabels(
                [_format_seqlen_label(seqlen) for seqlen in SEQLENS],
            )
