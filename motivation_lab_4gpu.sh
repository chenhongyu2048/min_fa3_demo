...
# Complete four-GPU MegaCP motivation batch. This script is self-contained.
...
RUN_ID=${RUN_ID:-motivation_4gpu_$(date -u +%Y%m%d_%H%M%S)}
...
T1_CASE_MANIFEST=${T1_CASE_MANIFEST:-motivation/t1_uniform_cases.json}
...
D1_TOPOLOGIES=${D1_TOPOLOGIES:-4:1,4:2}
...
usage() {
    cat <<'EOF'
Usage: ./motivation_lab_4gpu.sh

Runs complete CP4 T1/T2/T3/D1 experiments on GPU_IDS=0,1,2,3 by default.
D1 topologies are CP4/KVH1 (DCP4) and CP4/KVH2 (two DCP2 groups).
Use RUN_T1/RUN_T2/RUN_T3/RUN_D1=0 to resume or select a subset.
EOF
}
...
((${#GPU_LIST[@]} == 4)) || { echo "error: GPU_IDS must contain exactly four devices" >&2; exit 1; }
...
for topology in "${D1_TOPOLOGY_LIST[@]}"; do
    [[ $topology =~ ^4:(1|2)$ ]] || { echo "error: CP4 D1 topology must be 4:1 or 4:2" >&2; exit 1; }
done
...
run_distributed() {
    ...
    shift 2
    local -a command=("$TORCHRUN_BIN" --standalone --nproc_per_node=4 "$@")
    {
...
echo "Formal four-GPU motivation batch: $OUT_DIR"
echo "CP4 allocation: CUDA_VISIBLE_DEVICES=$GPU_IDS"
...
if [[ $RUN_T1 == 1 ]]; then
    ...
    for case_id in "${CASES[@]}"; do
        ...
        run_distributed "T1_${case_id}_CP4" "$OUT_DIR/t1_training_overlap/t1_${case_id}.log" \
...
if [[ $RUN_T2 == 1 ]]; then
    ...
    for case_id in "${CASES[@]}"; do
        ...
        run_distributed "T2_${case_id}_CP4" "$OUT_DIR/t2_training_step/t2_${case_id}.log" \
...
if [[ $RUN_T3 == 1 ]]; then
    run_local "T3_CP4" "$OUT_DIR/t3_load_balance/t3_cp4.log" \
        "$PYTHON_BIN" motivation/load_balance.py --datasets "$T3_DATASETS" \
        --target-tokens "$T3_TARGET_TOKENS" --num-cases "$T3_NUM_CASES" \
        --world-size 4 --qhead 32 --kvhead 8 --headdim 128 --seed "$T3_SEED" \
        --output-json "$OUT_DIR/t3_load_balance/t3_cp4.json"
else echo "T3 skipped"; fi

if [[ $RUN_D1 == 1 ]]; then
    for topology in "${D1_TOPOLOGY_LIST[@]}"; do
        kvh=${topology#*:}
        run_distributed "D1_CP4_KVH${kvh}" "$OUT_DIR/d1_decode_sm_trace/d1_cp4_kvh${kvh}.log" \
            motivation/decode_sm_trace.py --case-manifest "$D1_CASE_MANIFEST" \
            --qhead 32 --kvhead "$kvh" --tp-size 4 --num-comm-sm "$D1_NUM_COMM_SM" \
            --warmup "$D1_WARMUP" --iters "$D1_ITERS" \
            --output-json "$OUT_DIR/d1_decode_sm_trace/d1_cp4_kvh${kvh}.json" \
            --output-jsonl "$OUT_DIR/d1_decode_sm_trace/d1_cp4_kvh${kvh}.jsonl"
    done
else echo "D1 skipped"; fi

cat <<EOF

Completed formal four-GPU motivation batch: $OUT_DIR
